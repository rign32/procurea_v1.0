...
export class SlackNotificationsService {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.alertsChannelId = this.configService.get<string>('SLACK_ALERTS_CHANNEL_ID') || '';

        if (!this.botToken || !this.channelId) {
            this.logger.warn('SLACK_BOT_TOKEN or SLACK_CHANNEL_ID not configured — Slack notifications disabled');
        } else {
            this.logger.log('Slack notifications initialized');
        }
    }

    private async send(message: SlackMessage, channelOverride?: string): Promise<boolean> {
    ...
    async notifyApolloReply(email: string, name: string, sequenceName?: string): Promise<boolean> {
        return this.send({
            icon: '📤',
    ...
    async notifyRegistration(email: string, name: string, company?: string): Promise<boolean> {
        return this.send({
            icon: '🎉',
    ...
    async notifyFeedback(email: string, name: string, npsScore?: number, campaignName?: string): Promise<boolean> {
        return this.send({
            icon: '📝',
            ...
            fields: [
                { label: 'Użytkownik', value: name || email },
                { label: 'Email', value: email },
                ...(npsScore != null ? [{ label: 'NPS', value: `${npsScore}/10` }] : []),
    ...
    async notifyPayment(email: string, name: string, amount: number, currency: string, plan?: string): Promise<boolean> {
        const formatted = new Intl.NumberFormat('pl-PL', { style: 'currency', currency }).format(amount / 100);
        return this.send({
            icon: '💰',
    ...
    async notifySubscriptionCreated(email: string, name: string, amount: number, currency: string): Promise<boolean> {
        const formatted = new Intl.NumberFormat('pl-PL', { style: 'currency', currency }).format(amount / 100);
        return this.send({
            icon: '🏆',
    ...
    async notifySubscriptionCanceled(email: string, name: string): Promise<boolean> {
        return this.send({
            icon: '❌',
