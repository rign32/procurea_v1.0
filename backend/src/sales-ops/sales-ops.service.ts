...
export class SalesOpsService {
    ...
    async handleApolloReply(payload: {
        ...
        await this.slack.notifyApolloReply(payload.email, name, payload.sequence_name);
    }
    ...
    async handleRegistration(payload: {
        ...
        if (existingDeals.length > 0) {
            ...
            await this.attio.updateDealStage(existingDeals[0].recordId, 'rejestracja');
        } else {
            // Create new deal
            await this.attio.createDeal({
        ...
        await this.slack.notifyRegistration(payload.email, payload.name, payload.company);
    }
    ...
    async handleTallyFeedback(payload: {
        ...
        await this.slack.notifyFeedback(payload.email, name, payload.npsScore, payload.campaignName);
    }
    ...
    async handleStripeCheckout(payload: {
        ...
        this.logger.log(`Stripe checkout: ${payload.email} — ${payload.amount} ${payload.currency}`);
        ...
        if (deals.length > 0) {
            ...
            if (payload.amount) {
                extraValues.value = [{ currency_value: payload.amount / 100, currency_code: payload.currency?.toUpperCase() || 'USD' }];
            }
            if (payload.subscriptionId) {
                extraValues.stripe_subscription_id = [{ value: payload.subscriptionId }];
            }
            await this.attio.updateDealStage(dealId, 'platnosc', extraValues);
        }

        await this.slack.notifyPayment(payload.email, payload.name, payload.amount, payload.currency, payload.plan);
    }
    ...
    async handleSubscriptionCreated(payload: {
        ...
        if (deals.length > 0) {
            await this.attio.updateDealStage(deals[0].recordId, 'won', {
                ...
                value: [{ currency_value: payload.amount / 100, currency_code: payload.currency?.toUpperCase() || 'USD' }],
            });
        }

        await this.slack.notifySubscriptionCreated(payload.email, payload.name, payload.amount, payload.currency);
    }
