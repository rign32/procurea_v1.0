...
export class AttioService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (!this.apiKey) {
            this.logger.warn('ATTIO_API_KEY not configured — Attio integration disabled');
        } else {
    ...
    private async request(method: string, path: string, body?: any): Promise<any> {
        ...
        if (!res.ok) {
            ...
            this.logger.error(`Attio API error: ${res.status} ${method} ${path} — ${text}`);
            return null;
        }
    ...
    async createDeal(data: AttioDealData): Promise<string | null> {
        ...
        if (data.value) {
            values.value = [{ currency_value: data.value, currency_code: data.currency || 'USD' }];
        }
        ...
        const recordId = result?.data?.id?.record_id;
        if (recordId) {
            this.logger.log(`Attio deal created: ${data.name} (${data.stage}) → ${recordId}`);
        }
        return recordId || null;
    }
     ...
     */
    async updateDealStage(dealRecordId: string, stage: string, extraValues?: Record<string, any>): Promise<boolean> {
     ...
     */
    async addNote(parentRecordId: string, parentObjectSlug: string, title: string, content: string): Promise<boolean> {
        ...
        if (result) {
            this.logger.log(`Attio note added to ${parentObjectSlug}/${parentRecordId}: ${title}`);
        }
        return !!result;
    }
     ...
     */
    async findDealsByEmail(email: string): Promise<Array<{ recordId: string; name: string; stage: string }>> {
        // First find the person
        const searchResult = await this.request('POST', '/objects/people/records/query', {
            filter: {
        ...
        const dealsResult = await this.request('POST', '/objects/deals/records/query', {
            filter: {
