...
export class ObservabilityService {
    ...
    async recordEvent(
        ...
        try {
            ...
            if (severity !== 'info') {
                const slackSent = await this.sendToSlack(category, type, severity, options);
                if (slackSent) {
                    await this.prisma.observabilityEvent.update({
                        where: { id: event.id },
                        data: { slackSent: true },
                    }).catch(() => {}); // non-blocking
                }
            ...
            this.logger.log(`[${severity.toUpperCase()}] ${category}/${type}: ${options.title}`);
        } catch (err) {
    ...
    private async sendToSlack(
        ...
        if (options.userEmail) fields.push({ label: 'Email', value: options.userEmail });
        if (options.userId) fields.push({ label: 'User ID', value: options.userId });
        if (options.campaignId) fields.push({ label: 'Campaign', value: options.campaignId });
        if (options.message) fields.push({ label: 'Szczegóły', value: options.message.substring(0, 300) });

        // Route to appropriate channel: campaign/error → alerts, auth/conversion → sales
        const channelId = (category === 'campaign' || category === 'error')
