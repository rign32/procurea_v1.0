...
export class ObservabilitySchedulerService {
    ...
    await this.observability.recordEvent(
      ...
      "warning",
      {
        ...
        metadata: {
          ...
          emails: abandoned.map((u) => u.email),
        },
      },
    );
  ...
  private async checkNonConvertingUsers(): Promise<void> {
    for (const daysAgo of [2, 7]) {
      ...
      const nonConverting = planViews.filter(
        (pv) => !purchaserSet.has(pv.userId!),
      );

      if (nonConverting.length === 0) continue;

      const emailList = nonConverting
        .map((nc) => nc.userEmail || nc.userId)
        .join(", ");
      ...
      await this.observability.recordEvent(
        ...
        "warning",
        {
          ...
          metadata: {
            daysAgo,
            count: nonConverting.length,
            emails: nonConverting.map((nc) => nc.userEmail),
          },
        },
      );

      this.logger.log(
        `Non-converting reminder (${daysAgo}d): ${nonConverting.length} users`,
      );
    }
  }
  ...
  private async sendPlanViewsDigest(): Promise<void> {
    ...
    const uniqueEmails = [
      ...new Set(views.map((v) => v.userEmail).filter(Boolean)),
    ];
    const planDetails = views
      .map((v) => {
