...
let GoogleSearchService = GoogleSearchService_1 = class GoogleSearchService {
    ...
    apiKey = process.env.SERP_API_KEY || 'PLACEHOLDER_KEY';
    lastRequestTime = 0;
    MIN_DELAY_MS = 1500;
    ...
    async searchExtended(query, options, userId) {
        ...
        try {
            ...
            if (response.data.organic_results) {
                results = response.data.organic_results.map((r) => ({
                    title: r.title || '',
                    link: r.link || '',
                    snippet: r.snippet || ''
                }));
    ...
    async searchGlobal(query, userId) {
