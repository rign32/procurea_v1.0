...
<strong>2. RFQ orchestration.</strong> There is no native RFQ object. Some customers hack it with custom records or use the Opportunity object (a Sales feature) in reverse. Both break as soon as you need structured side-by-side comparison, response tracking, or revision history.

<strong>3. Multilingual outreach.</strong> NetSuite's email templates are single-language per template. Sending the same RFQ to 20 suppliers in Poland, Germany, and Turkey means you write three templates, maintain three glossaries, and translate manually. The Vendor Portal UI is English-plus-limited-localization — not native-language supplier-facing.

<strong>4. Certification tracking with expiry alerts.</strong> You can stuff ISO numbers into a custom field. NetSuite will not verify them, will not watch for expirations, and will not trigger a re-qualification workflow when a cert is within 90 days of lapsing.

<strong>5. Supplier risk scoring.</strong> Nothing native. NetSuite's vendor performance tracks PO-level metrics (fill rate, lead time), which is lagging. You cannot see financial-distress signals, geographic risk, or certification gaps before they matter.

<strong>6. Structured quote comparison.</strong> Vendor Bills compare invoices to POs, not quotes to quotes. If you receive 8 bids with different pricing tiers, MOQs, Incoterms, and payment terms, NetSuite has no canonical structure to normalize them. Buyers end up back in Excel.

Each of these gaps is a specialized capability. Trying to build them all in custom records + SuiteScript is technically possible and economically irrational — you would be recreating a procurement platform inside NetSuite. Most customers either accept the gaps and do the work in external tools, or integrate a purpose-built sourcing layer.`,
      bodyPl: `Każdy klient NetSuite, który próbuje prowadzić procurement na natywnych ekranach, uderza w te same sześć ścian.

<strong>1. Discovery dostawców.</strong> NetSuite nie ma mechanizmu szukania nowych dostawców. Rekord Vendor zakłada, że już ich znasz. Jeśli wchodzisz w nową kategorię (nowa geografia, nowa półka, nowy certyfikat), discovery dzieje się całkowicie poza NetSuite — zazwyczaj w Google, katalogach i sieciach konsultantów.

<strong>2. Orkiestracja RFQ.</strong> Nie ma natywnego obiektu RFQ. Część klientów hakuje to custom recordami albo używa Opportunity (feature sprzedażowy) w odwrotnym kierunku. Oba pękają, gdy potrzebujesz strukturyzowanego porównania side-by-side, trackingu odpowiedzi lub historii wersji.

<strong>3. Wielojęzyczny outreach.</strong> Szablony mailowe NetSuite są single-language na szablon. Wysłanie tego samego RFQ do 20 dostawców w Polsce, Niemczech i Turcji oznacza trzy szablony, trzy glosariusze i ręczne tłumaczenie. UI Vendor Portalu jest English-plus-ograniczona-lokalizacja — nie natywny do języka dostawcy.

<strong>4. Śledzenie certyfikatów z alertami wygaśnięć.</strong> Możesz wrzucić numery ISO w custom field. NetSuite ich nie zweryfikuje, nie popilnuje wygaśnięć i nie odpali workflow re-kwalifikacji, gdy certyfikat jest 90 dni przed wygaśnięciem.

<strong>5. Scoring ryzyka dostawcy.</strong> Nic natywnego. Vendor performance w NetSuite śledzi metryki na poziomie PO (fill rate, lead time), co jest opóźnione. Nie widzisz sygnałów dystresu finansowego, ryzyka geograficznego ani luk certyfikacyjnych, zanim zaczną boleć.
